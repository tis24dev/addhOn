...
| L5 | Light encoding assumed inverse (raw 2/1/0 = off/50/100) | Panel light behaves backwards. | Task 6 |
...
| V2 | `fan` and `light` were NOT added to `PLATFORMS` in Task 2 as Step 4 dictates. Each is added in the commit that creates its module (`fan` in Task 5, `light` in Task 6), so every commit stays runnable on a real instance. Enforced by `test_every_declared_platform_has_a_module`, which exists because the stubbed suite stays green with a platform listed and no module present. | Task 2 |
...
| V4 | `tests/test_command_dispatch.py` modified although listed in no task before Task 10. Its dormancy guard allow-lists dispatcher callers; this plan is what wakes the dispatcher. Now contains `air_purifier.py` (Task 2) and `fan.py` (Task 5); each further AP platform adds itself. | Tasks 2, 5 |
| V5 | `tests/test_entity_translation_keys.py::_collect_code_keys` hardcodes the platform list and had no `fan` entry, so an `entity.fan` block would have been entirely unchecked for stale or misspelled keys (the `extra` half of the parity assertion only runs for known platforms). Added in Task 5 although Task 11 owns the file. | Task 5 |
...
| P7 | Entity `unique_id` suffixes are permanent. The fan uses `purifier`; renaming any AP key in a later task orphans already-registered entities. | Task 5 |
