...
| Cooker hood | `HO` | fan, sensor, binary_sensor, switch, number | ✅ fan speed, errors, last work time + light, filter-cleaning alarm, filter cleaning in progress, running | ◑ fan speed (0…N steps from the live schema) + light switch, delayed switch-off switch and its delay in minutes. **No** remote way to reset the filter alarm (the parameter is `fixed`); the clock is writable in the schema but deliberately not exposed | ⚠️ no (schema of a real HADG6DS46BWIFI, issue #83) |
