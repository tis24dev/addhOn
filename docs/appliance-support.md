...
| Fridge / fridge-freezer | `REF` / `FR` | sensor, binary_sensor, select, number, switch, button | ✅ per-zone + upper/lower + ambient temperature, ambient humidity + per-zone doors, ice maker running, ice box full, energy saving, fault register. A My Zone drawer whose measured temperature arrives about 38 °C below reality is corrected, but only once a reading is impossible for the range its own setpoint declares (issue #75, cloud-side bias). A My Zone drawer also reports its MODE (0 °C fresh, quick cool, fruit and vegetables, ...) rather than a bogus target temperature: the drawer setpoint is a preset selector on these models, and the mode is resolved against the appliance's own program catalogue (issue #93) | ◑ per-zone target temperature, on a stepped cursor over exactly the degrees the appliance allows. The boost modes are **four independent switches** — auto set, super cool, super freeze, holiday — because that is what the appliance has: each writes one register and clears no other, so two can run at once, and switching one off clears only that one. A My Zone drawer gets a **mode select** with no *off* (the register is always in one of its modes), and each downloaded preset gets a **button** that sends it — a button because the appliance keeps no record of which preset ran. The old single program select survives only on a model where none of those can be built, e.g. one offering `iot_*` presets alone (issue #93). While a mode is running the appliance owns the zone setpoint and refuses to change it, so the setpoint control does too | ⚠️ no (the test fridge is offline; schema validated from its dump) |
| Freezer | `FRE` | sensor, binary_sensor, select, number, switch, button | ✅ same set as the fridge; gating drops the unused zones | ◑ same as the fridge | ⚠️ no |
