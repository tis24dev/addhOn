...
| Fridge / fridge-freezer | `REF` / `FR` | sensor, binary_sensor, select, number | ✅ per-zone + upper/lower + ambient temperature, ambient humidity + per-zone doors, ice maker running, ice box full, energy saving. A My Zone drawer whose measured temperature arrives about 38 °C below reality is corrected, but only once a reading is impossible for the range its own setpoint declares (issue #75, cloud-side bias) | ◑ per-zone target temperature + one program select (super cool, super freeze, holiday, the `iot_*` presets, and *off*) | ⚠️ no (the test fridge is offline; schema validated from its dump) |
